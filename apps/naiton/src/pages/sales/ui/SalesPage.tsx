...
export default function SalesPage() {
	...
	const createOrderMutation = useCreateMutation<CreateSalesOrderRequest, PostRequestResponse<SalesOrder>>({
		url: endpoints.SALES_ORDERS,
		onSuccess: (response) => {
			void queryClient.invalidateQueries({
				predicate: (query) =>
					typeof query.queryKey[0] === 'string' && query.queryKey[0].startsWith(endpoints.SALES_ORDERS)
			})
			showToast({
	...
	const filteredOrders = useMemo(() => {
		...
		return orders.filter((order) =>
			[order.id, order.customer, order.status].some((value) => value.toLowerCase().includes(normalizedSearch))
		)
			...
			>
				<DialogContent className='sm:max-w-[540px]'>
					<DialogHeader>
						...
						<DialogDescription>
							This posts to the MSW-backed sales endpoint and refreshes the lazy-loaded orders table.
						</DialogDescription>
					</DialogHeader>
					...
					>
						<div className='space-y-2'>
							...
							<Input id='sales-order-customer' placeholder='Praxis Amsterdam' {...register('customer')} />
							{errors.customer ? (
								<p className='text-destructive text-sm font-medium'>{errors.customer.message}</p>
							) : null}
						</div>
						...
						<div className='grid gap-4 sm:grid-cols-2'>
							<div className='space-y-2'>
								...
								/>
								{errors.amount ? <p className='text-destructive text-sm font-medium'>{errors.amount.message}</p> : null}
							</div>
							...
							<div className='space-y-2'>
								...
								/>
								{errors.status ? <p className='text-destructive text-sm font-medium'>{errors.status.message}</p> : null}
							</div>
						</div>
