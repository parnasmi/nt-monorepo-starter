...
const allowedProducts = ['sales', 'crm', 'wms', 'procurement', 'production', 'accounting', 'hrm', 'fms'] as const
...
const paginate = <T>(collection: T[], requestUrl: string) => {
	...
	const perPage = readPositiveInteger(url.searchParams.get('per_page') ?? url.searchParams.get('perPage'), 10)
