...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@repo/ui-kit/shadcn/card'
		...
		<section className='space-y-4 p-3 sm:p-5'>
			<Card className='naiton-surface rounded-[28px] border-white/70 bg-white/85 shadow-[0_12px_28px_rgba(15,23,42,0.05)] backdrop-blur'>
				<CardHeader className='gap-2 pb-4'>
					<Badge className='bg-primary/10 text-primary hover:bg-primary/10 w-fit rounded-full px-3 py-1'>Admin</Badge>
					<CardTitle className='text-3xl tracking-tight text-slate-800'>Cross-module command center</CardTitle>
					<CardDescription className='max-w-2xl text-sm leading-6 text-slate-500'>
						Snapshot KPIs help the suite shell feel alive from the first load while Sales and CRM wire into mock-backed
						module data.
					</CardDescription>
				</CardHeader>
			</Card>
					...
					return (
						<Card key={card.label} className='naiton-surface rounded-[24px] border-white/70'>
							<CardHeader className='flex flex-row items-start justify-between gap-4 pb-3'>
								<div>
									<CardDescription className='text-sm font-medium text-slate-500'>{card.label}</CardDescription>
									<CardTitle className='mt-3 text-4xl tracking-tight text-slate-800'>{card.value}</CardTitle>
								</div>
				...
				<Card className='naiton-surface rounded-[24px] border-white/70'>
					...
					<CardContent className='space-y-3'>
						{activityRows.map((row) => (
							<article key={row.title} className='rounded-[18px] border border-slate-200/80 bg-white/80 p-4'>
