...
- [ ] Run `pnpm init` at workspace root; set `"name": "naiton-workspace"`, `"private": true`.
- [ ] Add `packageManager: "pnpm@9.0.0"` and `engines.node: ">=18"` to root `package.json`.
- [ ] Create `pnpm-workspace.yaml`:
  ...
  packages:
    ...
    - 'packages/*'
  ```
- [ ] Install dev dependencies at root: `pnpm add -Dw turbo prettier prettier-plugin-tailwindcss husky lint-staged typescript@5.8`.
- [ ] Create `turbo.json`:
  ```json
  {
...
- [ ] Create `.prettierrc.json`:
  ...
  ```
- [ ] Create `.prettierignore` (ignore `node_modules`, `dist`, `.turbo`, `coverage`, `public`).
- [ ] Create `.gitignore` (standard Node + Vite + IDE).
- [ ] Initialise `git init` and make first commit: `chore: bootstrap monorepo`.
